...
var RetrieveActivitiesSchema = z.object({
  account: z.string(),
...
var retrieveActivitiesTemplate = `Respond with a JSON markdown block containing only the extracted values.
...
var activitiesAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    const { object } = await generateObject({
      ...
      schema: RetrieveActivitiesSchema
    });
    const retrieveActivitiesContent = object;
    const response = await getActivities({
      account: retrieveActivitiesContent.account,
      tag: !retrieveActivitiesContent.tag ? void 0 : [retrieveActivitiesContent.tag],
      type: !retrieveActivitiesContent.type ? void 0 : [retrieveActivitiesContent.type],
      network: retrieveActivitiesContent.network ? [retrieveActivitiesContent.network] : void 0,
      platform: retrieveActivitiesContent.platform ? [retrieveActivitiesContent.platform] : void 0,
...
var RetrieveNewsSchema = z.object({
  limit: z.number().max(100).default(10)
});
var retrieveNewTemplate = `Respond with a JSON markdown block containing only the extracted values.
...
var newsAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    const retrieveNewsContext = composeContext2({
      state: currentState,
    ...
    const { object } = await generateObject2({
      ...
      schema: RetrieveNewsSchema
    });
    const retrieveNewsContent = object;
    const news = await fetch(`https://ai.rss3.io/api/v1/ai_intel?limit=${retrieveNewsContent.limit}`).then((response) => response.json());
    if (callback) {
      await callback({
